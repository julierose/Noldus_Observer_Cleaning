...
obs_import <- read_excel("FILEPATH.xlsx", 
...
behavior <- obs_import %>% 
  ...
  #fish behavior names to the most recent fish behavior names. most of the time
  #this code should do nothing.
  mutate(across("Behavior", \(x) str_replace(x, "Station Keeping - Foraging", "Foraging"))) %>%
  mutate(across("Behavior", \(x) str_replace(x, "Scan and Pick", "Foraging"))) %>%
  mutate(across("Behavior", \(x) str_replace(x, "Ambush/Attack", "Foraging"))) %>%
  mutate(across("Behavior", \(x) str_replace(x, "Transit", "Foraging"))) %>%
  mutate(across("Behavior", \(x) str_replace(x, "Station Keeping - Shelter", "Shelter"))) %>%
  mutate(across("Behavior", \(x) str_replace(x, "Rapid Retreat into Cage", "Shelter"))) %>%
  mutate(across("Behavior", \(x) str_replace(x, "Retreat into Cage", "Shelter"))) %>%
  #this code removes the parentheses from the (not predator response) text
  mutate(across("Behavior", \(x) str_replace(x, " \\s*\\([^\\)]+\\)", ""))) %>%
  mutate(across("Behavior", \(x) str_replace(x, "Retreat to Cage", "Shelter"))) %>%
  mutate(across("Behavior", \(x) str_replace(x, "Resting", "Shelter"))) %>%
  mutate(across("Behavior", \(x) str_replace(x, "Agonistic Display", "Territoriality"))) %>%
  mutate(across("Behavior", \(x) str_replace(x, "Displacement", "Territoriality"))) %>%
  mutate(across("Behavior", \(x) str_replace(x, "Spawning", "Reproduction"))) %>%
  mutate(across("Behavior", \(x) str_replace(x, "Schooling", "Group Behavior"))) %>%
  mutate(across("Behavior", \(x) str_replace(x, "Aggregation", "Group Behavior"))) %>%
  mutate(across("Behavior", \(x) str_replace(x, "Escape from Cage", "Escape"))) %>%
  mutate(across("Behavior", \(x) str_replace(x, "Exiting cage", "Escape")))

View(behavior) #check to make sure everything looks right before exporting!
write_xlsx(behavior, "NEW FILEPATH.xlsx")